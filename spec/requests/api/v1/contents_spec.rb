...
RSpec.describe "Api::V1::Contents", type: :request do
  describe ' controller test cases ' do
    ...
    describe '#show' do
      it 'should give status 200' do
        get "/api/v1/projects/#{project1.id}/contents/#{content1.id}"
        expect(response.status).to eq(200)
        expect(response).to match_json_schema("content")
      end
    end
    ...
    describe '#index' do
      it 'should give status 200' do
        get "/api/v1/projects/#{project1.id}/contents"
        expect(response.status).to eq(200)
    ...
    describe '#destroy' do
      it 'should give status 401' do
        ...
        delete "/api/v1/projects/#{project2.id}/contents/#{content2.id}", headers: authentication_header(new_user)
        expect(response.status).to eq(401)
      end
      it 'should give status 200' do
        user2 = FactoryBot.create(:user, email: "test3@gmail.com") 
